...
if (isset($_POST['registerBtn'])){
    ...
    global $yhendus;
    $kask= $yhendus->prepare("SELECT * FROM kasutajad WHERE login=?");
        ...
        if ($username != "" && $passwd != "" && $passwd_again != ""){
            ...
            if ($passwd === $passwd_again){
                ...
                if ( strlen($passwd) >= 5 && strpbrk($passwd, "!#$.,:;()")){
                    ...
                    mysqli_query($yhendus, "INSERT INTO kasutajad (login, parool) VALUES ('$username', '$passwd_hashed')");
                    //echo "<script>alert('rrrr')</script>";
// verify the user's account was created
                    $query = mysqli_query($yhendus, "SELECT * FROM kasutajad WHERE login='{$username}'");
...
<form action="./registr.php" class="form" method="POST">
